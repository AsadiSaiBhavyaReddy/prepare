...
export default function Details() {
    ...
    const handleFilter = () => {
        let filtered = data.filter((item) => {
            ...
            if (maxPrice) {
                priceInRange = parseInt(item.price) <= parseInt(maxPrice);
            }
            if (maxRating) {
                ratingInRange = parseInt(item.rating) <= parseInt(maxRating);
            }
                ...
                >
                    <div style={{ padding: '10px', height: '100%', overflowY: 'auto' }}>
                        ...
                        {cartItems.map((item, index) => (
                            <div key={index}>
                                <Card variant="outlined" style={{ width: '250px', marginBottom: '10px' }}>
                                    <CardContent>
                                        ...
                                        <IconButton aria-label="delete" onClick={() => removeFromCart(index)} style={{ position: 'absolute', top: 0, right: 0 }}>
                                            <CloseIcon />
                                        </IconButton>
                                    </CardContent>
                                </Card>
